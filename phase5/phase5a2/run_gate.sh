...
kubectl -n mlflow port-forward svc/mlflow-server 5001:5000 >/tmp/mlflow-phase5a2-pf.log 2>&1 &
PF_PID=$!

for _ in $(seq 1 25); do
  if grep -q "Forwarding from" /tmp/mlflow-phase5a2-pf.log; then
    break
  fi
  sleep 1
done
...
if ! grep -q "Forwarding from" /tmp/mlflow-phase5a2-pf.log; then
  ...
  cat /tmp/mlflow-phase5a2-pf.log || true
  exit 1
fi
